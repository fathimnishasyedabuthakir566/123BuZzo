...
const AuthPage = ({ fixedRole, title, subtitle }: AuthPageProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: "",
    confirmPassword: "",
    role: fixedRole || ("user" as UserRole), // user = passenger
  });

  // Ensure formData.role stays in sync if fixedRole changes
  useEffect(() => {
    if (fixedRole) setFormData(prev => ({ ...prev, role: fixedRole }));
  }, [fixedRole]);
         ...
         <div className="w-full max-w-md">
            <div className="mb-10 text-center lg:text-left">
               ...
               <h3 className="text-3xl font-black text-white tracking-tight mb-2">
                 {title ? title : mode === "login" ? "Identity Authorization" : "New Registration Profile"}
               </h3>
               <p className="text-slate-500 text-sm font-medium">
                 {subtitle ? subtitle : mode === "login" ? "Enter your credentials to access the neural network" : "Create your identity to connect to the transit grid"}
               </p>
            </div>
